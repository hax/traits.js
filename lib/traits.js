...
var Trait = (function(){
   ...
   * augment(obj, trait)
   *
   * @param obj denotes the object to be augmented
   * @param trait a trait object to augment the object
   * @param option denotes whether check required
   * @returns void.
   * @throws 'Missing required property' the trait still contains a
   *         required property.
   * @throws 'Remaining conflicting property' if the trait still
   *         contains a conflicting property.
   *
   * added by github.com/hax, 2011-7-16
   *
   */
  function augment(self, trait, option) {
	if (option == null) option = {}
	if (!('required' in option)) option.required = true
    ...
    forEach(getOwnPropertyNames(trait), function (name) {
      ...
      // Note: it's OK for the prototype to provide the properties
      if (pd.required) {
        if (option.required && !(name in self)) {
          throw new Error('Missing required property: '+name);
        }
      } else if (pd.conflict) { // check for remaining conflicting properties
        throw new Error('Remaining conflicting property: '+name);
    ...
    defineProperties(self, properties);
	if ('new' in self) self['new'].call(self)
  }
